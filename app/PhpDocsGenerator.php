...
class PhpDocsGenerator implements EventInterface
{
    ...
    public function __invoke(EventDispatchInterface $event): void
    {
        ...
        foreach ($files as $file) {
            $json = $this->fileToArray($file);
            $slug = strtolower(str_replace('\\', '-', $json->className));
    ...
    private function fileToArray(string $filepath) : object
    {
        ...
        $collector = new class {
            public $github;
            public $className;
            public $classPhpDocDescription;
            public $methods = [];
            public $properties = [];
    
            public function addMethod($name, $visibility, $static, $returnType, $parameters, $description)
            {
                $this->methods[] = [
                    'name' => $name,
            ...
            public function addProperty($name, $visibility, $static, $type, $description, $default)
            {
                $this->properties[] = [
                    'name' => $name,
                    'visibility' => $visibility,
            ...
            public function enterNode(Node $node)
            {
                if ($node instanceof Node\Stmt\Class_) {
                    if (isset($node->namespacedName)) {
                        ...
                        $this->collector->className = $fullClassName;
                    } else {
                        // Fallback if namespacedName is not set
                        $this->collector->className = $node->name
                            ? $node->name->toString()
                            : null;
                    }
    
                    // Class doc comment
                    $this->collector->classPhpDocDescription = $this->getDocCommentText($node->getDocComment());
                }
    
                // If this is a class method, capture detailed method info
                if ($node instanceof Node\Stmt\ClassMethod) {
                    ...
                    $this->collector->addMethod($name, $visibility, $isStatic, $returnType, $parameters, $description);
                }
                ...
                if ($node instanceof Node\Stmt\Property) {
                    ...
                    foreach ($node->props as $prop) {
                        ...
                        $this->collector->addProperty($name, $visibility, $isStatic, $type, $description, $default);
                    }
                }
            }
